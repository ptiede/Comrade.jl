...
@recipe function f(acc::AbstractArrayConfiguration)
    ...
    aspect_ratio --> :equal
    label -->"Data"
    title --> "Frequency: $(first(acc[:Fr]) / 1.0e9) GHz"
    vcat(u / 1.0e9, -u / 1.0e9), vcat(v / 1.0e9, -v / 1.0e9)
end
