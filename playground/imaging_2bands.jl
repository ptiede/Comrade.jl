...
prior = (
          ...
          f = Uniform(0.2, 0.9),
          fg = Uniform(0.0, 1.0),
          lgamp1 = Comrade.GainPrior(distamp, scantable(damplo)),
          lgamp2 = Comrade.GainPrior(distamp, scantable(damphi))
        )
...
gt2 = Comrade.caltable(mms2(xopt))
plot(gt1, ylims=:none, layout=(3,3), size=(600,500))
plot!(gt2, ylims=:none, layout=(3,3), size=(600,500))
...
hchain, stats = sample(post, AHMC(;metric, autodiff=Val(:Zygote)), 4000; nadapts=3000, init_params=xopt)
