...
@sky function polimage(ftot, grid, config, pulse)
    ...
    c ~ GaussMarkovRandomField(σ, grid)
    p ~ ImageUniform(size(K))
    s ~ ImageSphericalUniform(size(K))
    img = ftot*K(c)
    return PoincareSphere2Map(img, p, s, cache)
end

@instrument function rime(dvis)
    scancache := jonescache()
    pgr       := station_tuple(dvis, Normal(0.0, 0.1); LM=Normal(0.0, 1.0))
    ...
    gaR, garat ~ CalPrior(dgR, ScanSeg(), dgrat, ScanSeg())

    J = Jones(Ga, Gp, D, R) do ga, gp, d, r
        adjoint(r)*ga*gp*d*r
    end

    dgr := station_tuple(dvis, Normal(0.0, 0.1))
    gaR ~ CalPrior(dgr, dvis, ScanSeg())
    gar ~ CalPrior(dgr, ScanSeg())

    JonesMatrix(Ga, exp.(gaR), exp.(gar .+ gaR))
