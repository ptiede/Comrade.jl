...
segs = sites_tuple(dvis, ScanSeg(); AA=FixedSeg(1.0 + 0.0im))
...
distphase = sites_tuple(sites(dvis)[2:end], DiagonalVonMises(0.0, inv(π^2)))
...
chain, stats = sample(rng, post, AHMC(;metric, autodiff=Val(:Zygote)), 1_000; n_adapts=3_000, initial_params=xopt)
#-
# !!! warning
