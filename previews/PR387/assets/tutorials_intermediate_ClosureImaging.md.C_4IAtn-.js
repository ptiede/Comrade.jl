import{_ as a,c as n,o as i,ai as p}from"./chunks/framework.sOM3bBO9.js";const l="/Comrade.jl/previews/PR387/assets/ClosureImaging-42.DD-h7-6H.png",e="/Comrade.jl/previews/PR387/assets/ClosureImaging-44.CGlGA3yY.png",t="/Comrade.jl/previews/PR387/assets/ClosureImaging-52.ChYvhKn9.png",h="/Comrade.jl/previews/PR387/assets/ClosureImaging-54.Ct0oY90-.png",u=JSON.parse('{"title":"Imaging a Black Hole using only Closure Quantities","description":"","frontmatter":{},"headers":[],"relativePath":"tutorials/intermediate/ClosureImaging.md","filePath":"tutorials/intermediate/ClosureImaging.md","lastUpdated":null}'),r={name:"tutorials/intermediate/ClosureImaging.md"};function k(c,s,o,d,g,E){return i(),n("div",null,s[0]||(s[0]=[p(`<h1 id="Imaging-a-Black-Hole-using-only-Closure-Quantities" tabindex="-1">Imaging a Black Hole using only Closure Quantities <a class="header-anchor" href="#Imaging-a-Black-Hole-using-only-Closure-Quantities" aria-label="Permalink to &quot;Imaging a Black Hole using only Closure Quantities {#Imaging-a-Black-Hole-using-only-Closure-Quantities}&quot;">​</a></h1><p>In this tutorial, we will create a preliminary reconstruction of the 2017 M87 data on April 6 using closure-only imaging. This tutorial is a general introduction to closure-only imaging in Comrade. For an introduction to simultaneous image and instrument modeling, see <a href="/Comrade.jl/previews/PR387/tutorials/intermediate/StokesIImaging#Stokes-I-Simultaneous-Image-and-Instrument-Modeling">Stokes I Simultaneous Image and Instrument Modeling</a></p><h2 id="Introduction-to-Closure-Imaging" tabindex="-1">Introduction to Closure Imaging <a class="header-anchor" href="#Introduction-to-Closure-Imaging" aria-label="Permalink to &quot;Introduction to Closure Imaging {#Introduction-to-Closure-Imaging}&quot;">​</a></h2><p>The EHT is one of the highest-resolution telescope ever created. Its resolution is equivalent to roughly tracking a hockey puck on the moon when viewing it from the earth. However, the EHT is also a unique interferometer. First, EHT data is incredibly sparse, the array is formed from only eight geographic locations around the planet. Second, the obseving frequency is much higher than traditional VLBI. Lastly, each site in the array is unique. They have different dishes, recievers, feeds, and electronics. Putting this all together implies that many of the common imaging techniques struggle to fit the EHT data and explore the uncertainty in both the image and instrument. One way to deal with some of these uncertainties is to not directly fit the data but instead fit closure quantities, which are independent of many of the instrumental effects that plague the data. The types of closure quantities are briefly described in <a href="/Comrade.jl/previews/PR387/vlbi_imaging_problem#Introduction-to-the-VLBI-Imaging-Problem">Introduction to the VLBI Imaging Problem</a>.</p><p>In this tutorial, we will do closure-only modeling of M87 to produce a posterior of images of M87.</p><p>To get started, we will load Comrade</p><div class="language-julia vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">julia</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">using</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> Comrade</span></span></code></pre></div><p>Pyehtim loads eht-imaging using PythonCall this is necessary to load uvfits files currently.</p><div class="language-julia vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">julia</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">using</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> Pyehtim</span></span></code></pre></div><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>    CondaPkg Found dependencies: /home/runner/.julia/packages/DimensionalData/M9vEC/CondaPkg.toml</span></span>
...
<span class="line"><span>Resolved 35 packages in 52ms</span></span>
<span class="line"><span>Installed 29 packages in 60ms</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">skymeta </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">=</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> (;mimg </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">=</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> imgpr</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">./</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">flux</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">(imgpr));</span></span></code></pre></div><p>Now we can finally form our image prior. For this we use a heirarchical prior where the direct log-ratio image prior is a Gaussian Markov Random Field. The correlation length of the GMRF is a hyperparameter that is fit during imaging. We pass the data to the prior to estimate what the maximumal resolutoin of the array is and prevent the prior from allowing correlation lengths that are much small than the telescope beam size. Note that this GMRF prior has unit variance. For more information on the GMRF prior see the <a href="/Comrade.jl/previews/PR387/api#Comrade.corr_image_prior"><code>corr_image_prior</code></a> doc string.</p><div class="language-julia vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">julia</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">cprior </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">=</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> corr_image_prior</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">(grid, dlcamp)</span></span></code></pre></div><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>HierarchicalPrior(</span></span>
