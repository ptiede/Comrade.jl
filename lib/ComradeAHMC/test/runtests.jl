...
@testset "ComradeAHMC.jl" begin
    ...
    hchain, hstats = sample(post, s1, 3_000; n_adapts=2_000, progress=false, init_params=x0)
    hchain, hstats = sample(post, s2, 3_000; n_adapts=2_000, progress=false, init_params=x0)
    out = sample(post, s2, 3_000; n_adapts=2_000, saveto=ComradeAHMC.DiskStore(name=joinpath(@__DIR__, "Test")), init_params=x0)
    ...
    # hchain, hstats = sample(post, s2, Comrade.AbstractMCMC.MCMCThreads(), 3_000, 2; n_adapts=2_000, progress=false,init_params=[x0,x0])
