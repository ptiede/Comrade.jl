...
@testset "ComradeAHMC.jl" begin
    ...
    hchain, ends = sample(post, s1, 3_000; nadapts=2_000, progress=false)
    hchain, ends = sample(post, s1, 3_000; nadapts=2_000, progress=false, init_params=x0)
    hchain, ends = sample(post, s2, 3_000; nadapts=2_000, progress=false, init_params=x0)

    hchain, ends = sample(post, s2, Comrade.AbstractMCMC.MCMCThreads(), 3_000, 2; nadapts=2_000, progress=false)
    hchain, ends = sample(post, s2, Comrade.AbstractMCMC.MCMCThreads(), 3_000, 2; nadapts=2_000, progress=false,init_params=[x0,x0])

    cpost = asflat(post)
    l0 = logdensityof(cpost, Comrade.HypercubeTransform.inverse(cpost, x0))
    @test 10*l0  < mean(hchain.sample_stats.log_density[2000:end])
